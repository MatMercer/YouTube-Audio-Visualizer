...
function barsScene() {
    ...
    inst.render = function(c, g, r, fd, td) {
        //the animation

        //a constant to calculate the bar width responsively
        widthConstant = (100 / (fd.length - st.settings.scenes.bars.excludeRatio));
        ...
        for (i = 0; i < fd.length - st.settings.scenes.bars.excludeRatio; i++) {
            //the bar width based on the widthConstant
            barWidth = widthConstant * (r.width / 100);

            //draw the rectangle
            g.drawRect(barWidth * i, r.height - (r.height * fd[i]), barWidth, r.height * fd[i]);
        }
...
function oscilloscopeScene() {
    ...
    inst.render = function(c, g, r, fd, td) {
        ...
        if (st.settings.scenes.oscilloscope.backgroundOpacity) {
            drawBackground(g, r, st.settings.scenes.oscilloscope.backgroundColor, st.settings.scenes.oscilloscope.backgroundOpacity);
        }

        //move the drawer to start point
        g.moveTo(0, (r.height / 2) + (r.height * td[0] / 1.5));

        //the line style
        g.lineStyle(st.settings.scenes.oscilloscope.lineWidth, st.settings.scenes.oscilloscope.lineColor);
        ...
        for (i = 1; i < td.length; i++) {
            ...
            lineWidth = widthConstant * (r.width / 100);

            //draw the line, in the next loop, the drawer will be at the given coord
            g.lineTo(lineWidth * i, (r.height / 2) + (r.height * td[i] / 1.5));
        }
...
function monsterYTAVScene() {
    ...
    inst.render = function(c, g, r, fd, td) {
        ...
        //a width constant
        widthConstant = (100 / inst.freqData.length);
...
function audioVisualizer(width, height, containerSelector, sourceSelector, playerSelector, analyserData) {
    ...
    inst.render = function() {
        ...
        //loop based in the browser framerate
    ...
    inst.updateAnalyserConfig = function(config) {
        inst.analyser.smoothingTimeConstant = st.settings.scenes.get(scenes[st.settings.global.sceneIndex].name.toLowerCase()).smooth || 0.8;
        inst.setFFT(st.settings.scenes.get(scenes[st.settings.global.sceneIndex].name.toLowerCase()).fftSize || 256);
    };
